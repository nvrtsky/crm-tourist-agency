...
-   **Leads Module**: Offers comprehensive lead management with dual table/Kanban views, lead-to-tourist data separation, detailed passport data for individual tourists via `leadTourists` table, enhanced lead conversion processes, and automatic creation of initial tourist entries. Features manual color tagging plus automatic color highlighting (green for won/converted, red for lost statuses, with manual colors taking precedence). Kanban view displays status-based badge colors: gray for new/contacted, amber/gold for qualified, dark green for converted, red for lost - all meeting WCAG AA accessibility standards.
