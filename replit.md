...
### Recent Changes
-   **Sticky Column Fix for EventSummary (Nov 2025)**: Fixed content bleed-through issue where scrolling content from "Данные туриста" column was visible through sticky columns (№ and ФИО) during horizontal scroll. Implemented explicit opaque backgrounds (`bg-white dark:bg-slate-950`) instead of semi-transparent theme colors, precise width calculation (w-[56px] for № column), exact positioning (left-[56px] for ФИО column), and added subtle box-shadow for visual separation. Ensures complete opacity and clear boundary between sticky and scrolling regions in both light and dark themes.
