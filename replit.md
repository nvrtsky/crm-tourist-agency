...
-   **Conditional Personal Data Validation (Nov 2025)**: Modified `calculateTouristDataCompleteness` in lib/utils.ts to require email and phone only for primary tourists (isPrimary === true). For secondary family members and group participants, only ФИО and date of birth are required for "complete" personal data status. Uses explicit boolean check to handle edge cases safely.
