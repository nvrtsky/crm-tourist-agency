...
-   **Wazzup24 Integration**: WhatsApp chat integration via Wazzup24 API. Features include embedded chat iframe in lead cards, automatic contact sync with phone normalization, and user mapping for proper permissions. **User Mapping**: CRM users can be mapped to existing Wazzup24 users via `wazzup24UserId` field in Settings > Wazzup24 > "Привязка пользователей". This enables managers to write messages (not just view) by linking to their pre-existing Wazzup24 accounts with proper roles (Manager/Supervisor). Uses API scope "card" with filter for contact-specific chats.
